...
- [`statusline`](#statusline-プラグイン): ブランチ・モデル・コスト・所要時間・差分行数・名言（任意）を表示するシェル用ステータスラインをインストール
- [`gh`](#gh-プラグイン): 変更を分析し、適切に構造化された PR を自動生成するプラグイン
- [`git`](#git-プラグイン): git-flow スタイルのブランチ作成とコミットメッセージ自動生成を行う Git ワークフロー自動化ツール
...
- **名言表示（任意）**: 5 分ごとに新しいプログラミング名言を表示（キャッシュ機能付き）。`--no-quotes` で無効化可能。
...
- **Git-Flow ブランチ作成**: タスク説明に基づいて適切な git-flow ブランチ（feature/bugfix/hotfix/release）を自動作成
- **インテリジェントなブランチ命名**: わかりやすく一貫性のあるブランチ名を自動生成
...
- **自動プッシュ**: ブランチ作成やコミット後にオプションでリモートへ自動プッシュ
...
statusline プラグインは、Claude Code セッションに関する豊富な情報を表示するカスタマイズ可能なステータスラインスクリプトをインストールします。現在の git ブランチ、AI モデル、セッションコスト、所要時間、変更行数、（オプションで）名言を表示します。
...
```
`~/.claude/settings.json` の `statusLine.command` に `bash ~/.claude/scripts/statusline.sh --no-quotes` が書き込まれ、名言セクションが表示されなくなります。
...
#### 後から名言を非表示にする（再インストール不要）

- 一時的に無効化：プレビューやスクリプト実行時に環境変数を付与します。

```bash
CLAUDE_STATUSLINE_NO_QUOTES=1 ~/.claude/scripts/statusline.sh
```

- 恒久的に無効化：`~/.claude/settings.json` のコマンドに `--no-quotes` を追加します。
...
もしくはインストーラを `--force --no-quotes` で再実行すると自動で設定が更新されます。
...
git プラグインは、git-flow スタイルの操作で Git ワークフローを自動化します：
- タスク説明に基づいて git-flow ブランチ（feature/bugfix/hotfix/release）を作成
- Conventional Commit メッセージを生成し、変更を自動的にプッシュ

### コマンド

git プラグインは 2 つの主要なコマンドを提供します：

#### 1. ブランチ作成（`/git:create-branch`）

タスク説明に基づいて新しい git-flow スタイルのブランチを作成します。

**使い方：**
```bash
/git:create-branch "ユーザー認証を追加"
/git:create-branch "ログインエラーを修正" --base main
/git:create-branch "緊急のセキュリティパッチ" --type hotfix --no-push
```

**コマンドの動作：**
1. タスク説明を分析してブランチタイプを判定（feature/bugfix/hotfix/release）
2. わかりやすい説明的なブランチ名を生成
3. 適切なベース（develop/main/master）からブランチを作成
4. オプションでリモートへプッシュ
...
**フラグ：**
- `--base <branch>`: ベースブランチを指定（デフォルト：develop/main/master を自動検出）
- `--type <type>`: ブランチタイプを強制（feature/bugfix/hotfix/release）
- `--no-push`: ブランチをリモートへプッシュしない

**使用例：**
```bash
# 自動検出による feature ブランチ
/git:create-branch "OAuth2 認証を追加"
# 結果: feature/oauth2-認証を追加

# 特定のベースからの bugfix
/git:create-branch "モバイルでのログインクラッシュを修正" --base main
# 結果: bugfix/モバイルでのログインクラッシュを修正

# プッシュしない hotfix
/git:create-branch "API のセキュリティ脆弱性" --type hotfix --no-push
# 結果: hotfix/api-のセキュリティ脆弱性
```

#### 2. コミット（`/git:commit`）

変更を分析し、git-flow スタイルのコミットメッセージを作成してプッシュします。
